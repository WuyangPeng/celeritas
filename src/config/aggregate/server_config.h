#pragma once

#include "config/config_constant.h"
#include "config/local/server_network_config.h"

#include <string>
#include <vector>

namespace celeritas
{
    class server_config
    {
    public:
        using class_type = server_config;
        using server_network_container = std::vector<server_network_config>;
        using container_const_iterator = server_network_container::const_iterator;

        server_config() noexcept = default;

        server_config(std::string instance_id, std::string service_name, server_network_container server_network_config, std::string game_server_id, std::string host, int worker_pool_size, int datacenter_id, int worker_id, int frame, bool load_game_config) noexcept;

        [[nodiscard]] container_const_iterator begin() const noexcept;

        [[nodiscard]] container_const_iterator end() const noexcept;

        [[nodiscard]] std::string get_instance_id() const;

        [[nodiscard]] std::string get_game_server_id() const;

        [[nodiscard]] std::string get_service_name() const;

        [[nodiscard]] bool is_service_registry_server() const;

        [[nodiscard]] int get_worker_pool_size() const noexcept;

        [[nodiscard]] std::string get_host() const;

        [[nodiscard]] int get_datacenter_id() const noexcept;

        [[nodiscard]] int get_worker_id() const noexcept;

        [[nodiscard]] int get_frame() const noexcept;

        [[nodiscard]] bool is_load_game_config() const noexcept;

    private:
        std::string instance_id_;
        std::string service_name_;
        server_network_container server_network_config_;
        std::string game_server_id_;
        std::string host_;
        int worker_pool_size_ = default_worker_pool_size;
        int datacenter_id_ = 0;
        int worker_id_ = 0;
        int frame_ = 0;
        bool load_game_config_ = false;
    };
}
