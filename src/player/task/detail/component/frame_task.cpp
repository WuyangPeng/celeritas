...
celeritas::frame_task::frame_task(player_state* player_state) noexcept
    : base_type{ player_state }
{
}

celeritas::task::task_component_type celeritas::frame_task::get_task_component_type() const
{
    return task_component_type::frame;
}

celeritas::task::void_awaitable_type celeritas::frame_task::on_db_analysis(const const_app_config_shared_ptr& app_config)
{
    const auto task_config = game_config::get_instance().get_game_tables()->get_pretreatment_config()->get_task_config();
    const auto optional_tasks = task_config->get_tasks_by_type(task_component_type::frame);
