...
celeritas::activate_item::activate_item_shared_ptr celeritas::activate_item::create(protobuf_handle_parameter_shared_ptr handle_parameter, player_state_shared_ptr player_state, request_type request)
{
    return std::make_shared<class_type>(std::move(handle_parameter), std::move(player_state), std::move(request));
}

celeritas::activate_item::activate_item(protobuf_handle_parameter_shared_ptr handle_parameter, player_state_shared_ptr player_state, request_type request)
...
celeritas::player_service_base::void_awaitable_type celeritas::activate_item::response()
{
...
void celeritas::activate_item::send_activate_item_response()
{
    ...
    response.mutable_celeritas_response()->mutable_client()->mutable_player()->mutable_item()->mutable_activate_item();

    if (!player_state->write(gateway_type.data(), player_state->get_instance_id(), header, response))
    {
        LOG_CHANNEL(player_channel, error) << "send activate item response error.";
    }
}
