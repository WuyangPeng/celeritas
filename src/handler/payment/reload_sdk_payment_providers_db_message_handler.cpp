...
bool celeritas::reload_sdk_payment_providers_db_message_handler::handle_concrete(const protobuf_handle_parameter& handle_parameter, const message_type& current_message, const message_registry_weak_ptr& message_registry)
{
    ...
    const auto sdk_payment_provider = app_sdk_payment_providers::get_instance().get_sdk_providers(current_message.sdk_id());
    ...
    application_loader_shared_ptr->register_handler(std::make_shared<notify_http_message_handler>(sdk_payment_provider.get_sdk_id(), sdk_payment_provider.get_http_suffix()));
