...
namespace celeritas
{
    class login_servers_http_message_handler final : public http_base_message_handler
    {
    public:
        using class_type = login_servers_http_message_handler;
        using base_type = http_base_message_handler;

        login_servers_http_message_handler() noexcept = default;
