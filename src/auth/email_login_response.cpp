...
celeritas::email_login_response::email_login_response(const game_error_type code, std::string message, std::string token, const int64_t expire_milliseconds)
...
celeritas::email_login_response celeritas::tag_invoke(email_login_response_tag, const http_response::json_value& value)
{
    return email_login_response{ tag_invoke(token_http_response_tag{}, value) };
}

void celeritas::tag_invoke(const boost::json::value_from_tag tag, email_login_response::json_value& value, const email_login_response& email_login_response)
{
    tag_invoke(tag, value, email_login_response::bass_type{ email_login_response.get_code(), email_login_response.get_message(), email_login_response.get_token(), email_login_response.get_expire_milliseconds() });
}