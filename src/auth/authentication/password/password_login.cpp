...
#include "auth/detail/password_login_parameter.h"
...
celeritas::password_login::void_awaitable_type celeritas::password_login::response()
{
    ...
    const auto key = std::make_shared<basis_database_container>(basis_database_container::object_container{
        { account_bind::account_type_describe, static_cast<int>(account_type::password) },
    ...
    const auto optional_account_bind = co_await mysql_pool->select_one(
        account_bind::get_select(database_type::mysql, key), account_bind::get_database_field_container());
...
celeritas::password_login::void_awaitable_type celeritas::password_login::login(
    ...
    const std::string& password) const
{
    if (database_entity_change)
    {
        ...
        if (auto optional_account = co_await mysql_pool->select_one(
    ...
    const auto account = co_await create_new_account(app_id, auth_key, password, account_type::password,
                                                     sdk_process_type::null, "password", redis_pool, get_app_config());

    co_return co_await login(redis_pool, account, true);
}

celeritas::password_login::void_awaitable_type celeritas::password_login::login(
    const database_pool_shared_ptr& redis_pool, const account& account, const bool is_new_account) const
{
    if (const auto session_token = co_await create_session_token(account, is_new_account, redis_pool))
    {
        write(password_login_response{
