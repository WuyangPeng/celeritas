...
#include "auth/detail/send_sms_parameter.h"
#include "auth/detail/send_sms_to_providers.h"
...
celeritas::send_sms::void_awaitable_type celeritas::send_sms::response() {
    ...
    if (auto sms_limit = co_await redis_pool->select_one(sms_limit::get_select(database_type::redis, phone),
                                                         sms_limit::get_database_field_container())) {
