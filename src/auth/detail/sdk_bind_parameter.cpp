...
bool celeritas::sdk_bind_parameter::is_failure() const
{
    return response_.has_value();
}

celeritas::sdk_bind_response celeritas::sdk_bind_parameter::get_response() const
{
    return *response_;
}

std::string celeritas::sdk_bind_parameter::get_token() const
{
    return token_;
}
...
void celeritas::sdk_bind_parameter::init()
{
    ...
    {
        response_ = sdk_bind_response{ *http_response };
    }

    if (const auto hmac_sha256 = hmac_sha256::calculate_with_args(get_apps().get_app_secret(), get_app_id(), get_sdk_token(), token_, get_timestamp());
        hmac_sha256 != get_sign())
    {
        response_ = sdk_bind_response{ game_error_type::sign_error };
    }
}