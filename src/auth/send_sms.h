...
namespace celeritas
{
    ...
    private:
        [[nodiscard]] static std::string calculate_hmac_sha256(int64_t app_id, const std::string& phone, int64_t timestamp, const std::string& secret_key);

        void send_sdk_sms(const sms_code& sms_code, const apps& apps);
