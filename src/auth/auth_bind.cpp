...
{
    co_return co_await bind(account, app_id, auth_key, "", account_type, mysql_pool);
}

celeritas::auth_bind::bool_awaitable_type celeritas::auth_bind::bind(account& account, const int64_t app_id, const std::string& auth_key, const std::string& password, account_type account_type, const database_pool_shared_ptr& mysql_pool) const
{
    if (password.empty())
    {
