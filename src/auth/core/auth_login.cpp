...
celeritas::auth_login::optional_database_entity_change_awaitable_type celeritas::auth_login::get_optional_account(
    int64_t app_id,
    ...
    const auto key = std::make_shared<basis_database_container>(basis_database_container::object_container{
        {account::device_id_describe, device_id},
...
celeritas::auth_login::account_awaitable_type celeritas::auth_login::create_new_account(const int64_t app_id,
    ...
    const auto account_id = snowflake_generator::get_instance().generate(
...
celeritas::auth_login::account_awaitable_type celeritas::auth_login::create_new_account(const int64_t app_id,
    ...
    const database_pool_shared_ptr &redis_pool,
    const const_app_config_shared_ptr &app_config) {
    co_return co_await create_new_account(app_id, auth_key, "", account_type, sdk_process_type, account_name_prefix,
...
celeritas::auth_login::account_awaitable_type celeritas::auth_login::create_new_account(int64_t app_id,
    ...
    account.set_account_name(
        get_sdk_process_type_description(sdk_process_type) + account_name_prefix + "_" + std::to_string(account_id));
    account.set_create_time(time_helper::get_current_milliseconds());
    account.set_status(static_cast<int>(account_status_type::normal));
    ...
    const auto account_bind_id = snowflake_generator::get_instance().generate(
        server_config.get_datacenter_id(), server_config.get_worker_id());
