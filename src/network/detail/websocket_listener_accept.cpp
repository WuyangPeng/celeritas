...
celeritas::listener_accept::void_awaitable_type celeritas::websocket_listener_accept::accept_connections()
{
    while (is_running())
    {
        try
...
celeritas::listener_accept::void_awaitable_type celeritas::websocket_listener_accept::handle_connection()
{
    // 等待新连接
...
void celeritas::websocket_listener_accept::start_new_session(socket_type socket)
{
    ...
    const auto session = std::make_shared<websocket_session>(std::move(socket), current_session_id, game_server_id_, session_callback_);
    add_session(session);
