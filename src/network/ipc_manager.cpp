...
boost::asio::awaitable<std::shared_ptr<celeritas::session_base<boost::asio::local::stream_protocol::socket>>> celeritas::ipc_manager::connect_to(const std::string& remote_path)
{
    try
    {
        ...
        co_return std::make_shared<session_base<boost::asio::local::stream_protocol::socket>>(std::move(socket), message_handler_);
    }
    catch (const boost::system::system_error& e)
    {
