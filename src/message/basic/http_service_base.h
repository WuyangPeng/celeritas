...
namespace celeritas
{
    ...
    {
    public:
        using class_type = http_service_base;
        using void_awaitable_type = boost::asio::awaitable<void>;
        using any_io_executor = boost::asio::any_io_executor;
        using io_context_type = boost::asio::io_context;
        using http_handle_parameter_shared_ptr = std::shared_ptr<http_handle_parameter>;

        explicit http_service_base(http_handle_parameter_shared_ptr handle_parameter);

        virtual ~http_service_base() noexcept = default;

        http_service_base(const http_service_base& rhs) = default;

        http_service_base& operator=(const http_service_base& rhs) = delete;

        http_service_base(http_service_base&& rhs) noexcept = default;

        http_service_base& operator=(http_service_base&& rhs) noexcept = delete;
