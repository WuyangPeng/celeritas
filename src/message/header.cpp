...
celeritas::header::header(const proto::empty_message_header& empty_message_header)
    : rpc{}, user_id{}, code{}
{
}
...
celeritas::header::message_shared_ptr celeritas::header::get_message() const
{
    if (user_id > 0)
    {
        auto server_message_header = std::make_shared<proto::server_message_header>();
    ...
    if (rpc > 0)
    {
        auto client_message_header = std::make_shared<proto::client_message_header>();
    ...
    return std::make_shared<proto::empty_message_header>();
}