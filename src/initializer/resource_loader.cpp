...
void celeritas::resource_loader::initialize(io_context_type &io_context,
...
bool celeritas::resource_loader::write(const std::string &server_type, const header &header,
                                       const protobuf_message &request) const {
    auto to_write = false;
...
void celeritas::resource_loader::process_service_registry_by_duration() {
    proto::celeritas request{};
    auto *server_register = request.mutable_celeritas_request()->mutable_service()->mutable_registry()->
            mutable_server_register();
...
celeritas::resource_loader::health_check_level_awaitable_type
celeritas::resource_loader::get_health_check_level() const {
...
void celeritas::resource_loader::initialize_server_resource(io_context_type &io_context,
    ...
    for (const auto &element: server) {
        const auto listener = server_resource_loader::loader_server(io_context, server, element,
                                                                    network_message_callback);
...
void celeritas::resource_loader::initialize_service_registry_resource(io_context_type &io_context,
                                                                      const network_message_callback_weak_ptr &
                                                                      network_message_callback) {
    ...
    if (!is_service_registry_) {
        ...
        for (const auto &element: service_registry | std::views::values) {
            if (element.get_name() != instance_id) {
                const auto client = service_registry_loader::loader_service_registry(
                    io_context, element, network_message_callback, game_server_id, service_registry_type.data());
...
void celeritas::resource_loader::modify_service_registry_resource(io_context_type &io_context,
                                                                  const network_message_callback_weak_ptr &
                                                                  network_message_callback, int index) {
...
void celeritas::resource_loader::start_check_tcp_clients_timer(io_context_type &io_context) {
    check_tcp_clients_timer_ = std::make_unique<check_tcp_clients_timer>(
        io_context, check_tcp_clients_seconds, shared_from_this());

    check_tcp_clients_timer_->start();
}

void celeritas::resource_loader::start_service_registry_timer(io_context_type &io_context) {
    service_registry_timer_ = std::make_unique<service_registry_timer>(io_context, service_registry_seconds,
                                                                       shared_from_this());
...
celeritas::resource_loader::tcp_client_shared_ptr celeritas::resource_loader::get_random_client(
    io_context_type &io_context, const network_message_callback_weak_ptr &network_message_callback,
    ...
    const auto server = app_config_->get_server_config();
    return service_registry_loader::loader_service_registry(io_context, iter->second, network_message_callback,
                                                            server.get_game_server_id(), service_registry_type.data());
}