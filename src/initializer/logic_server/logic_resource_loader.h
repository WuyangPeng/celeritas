...
namespace celeritas
{
    class logic_resource_loader final : public resource_loader
    {
    public:
        using class_type = logic_resource_loader;
        using base_type = resource_loader;

        explicit logic_resource_loader(std::string_view server_type, const_app_config_shared_ptr app_config);
