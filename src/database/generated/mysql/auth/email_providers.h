...
namespace celeritas
{
    class email_providers final : public database_entity
    {
    public:
        using class_type = email_providers;
        using base_type = database_entity;

        explicit email_providers(const database_entity_change& entity);

        email_providers(database_type database_type, const database_entity_change& entity);

        email_providers(database_type database_type, traits::param_type::int64_type provider_id);

        [[nodiscard]] traits::int64_type get_provider_id() const noexcept;
        ...
        void set_provider_id(traits::param_type::int64_type provider_id);
        ...
        static constexpr std::string_view database_name{ "email_providers" };

        static constexpr std::string_view provider_id_describe{ "provider_id" };
