...
celeritas::redis_sorted_set_commands::int_awaitable_type celeritas::redis_sorted_set_commands::async_add_many(const std::string& key, const sorted_set_member_score_container& members) const
{
    ...
    auto command = "ZADD " + get_prefixed_key(key);

    for (const auto& item : members)
    {
        command += " " + std::to_string(item.get_score()) + " " + get_quoted_value_command(item.get_member());
    }
...
{
    const auto command = "ZREM " + get_prefixed_key(key) + " " + get_quoted_value_command(member.get_member());
...
{
    const auto command = "ZREM " + get_prefixed_key(key) + " " + get_values_command(members);
...
{
    const auto command = std::string("ZSCORE ") + get_prefixed_key(key) + " " + get_quoted_value_command(member);
...
{
    const auto command = std::string("ZCARD ") + get_prefixed_key(key);
...
{
    const auto command = std::string("ZINCRBY ") + get_prefixed_key(key) + " " + std::to_string(increment) + " " + get_quoted_value_command(member);
...
{
    auto command = "ZRANGE " + get_prefixed_key(key) + " " + std::to_string(start) + " " + std::to_string(end);
    if (with_scores)
    {
        command += " WITHSCORES";
    }
...
{
    auto command = "ZREVRANGE " + get_prefixed_key(key) + " " + std::to_string(start) + " " + std::to_string(end);
    if (with_scores)
    {
        command += " WITHSCORES";
    }
...
{
    const auto command = std::string("ZRANK ") + get_prefixed_key(key) + " " + get_quoted_value_command(member);
...
{
    const auto command = std::string("ZREVRANK ") + get_prefixed_key(key) + " " + get_quoted_value_command(member);
