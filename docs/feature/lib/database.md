...
* **🧱 基础数据单元 (`basis_database`)**
    ...
    - **特点**：内部使用 `std::variant` 存储不同类型的值，并记录字段名和数据类型 (`database_data_type`)。提供了
...
* **📜 配置表基类 (`config_table_base`)**
    - **作用**：定义了所有配置表（`Config Table`）的通用抽象接口。
    - **特点**：
        - **多态性**：作为 `config_manager` 管理的所有配置表的基类，实现了统一管理。
        - **接口定义**：定义了 `load_all()` 和 `load_one()` 等纯虚函数，强制所有具体的配置表实现自己的加载逻辑。

* **📚 通用配置表 (`config_table<T>`)**
    - **作用**：一个可重用的模板类，用于实现具体的配置表。
    - **特点**：
        - **泛型实现**：封装了从数据库加载数据并缓存为 `std::shared_ptr<const T>` 的通用逻辑。
        - **易于扩展**：要添加一个新的配置类型，只需实例化此模板（例如 `config_table<new_config_type>`）并在 `config_manager` 中注册即可，无需编写重复的加载和缓存代码。

## document (文档)

* **📄 文档数据库实体 (`document`)**
    - **作用**: 此目录包含为文档型数据库（如`MongoDB`）自动生成的实体类。这些类将数据库中的文档映射为`C++`对象，简化了数据访问和操作。
    - **特点**:
        - 每个文件对应一个数据库集合（`Collection`）。
        - 提供了类型安全的成员变量和方法来访问文档字段。

## generated (生成代码)

* **🤖 关系型数据库实体 (`generated`)**
    - **作用**: 此目录存放为数据库自动生成的实体类。这些代码将数据库映射为`C++`对象。
    - **特点**:
        - 包含了与数据库结构对应的类定义。
        - 简化了查询结果到`C++`对象的转换过程。
        - 由代码生成工具根据数据库的`json文件`自动创建。
    - **注意**: 此目录下的文件不应手动编辑。应通过修改数据库`json文件`并重新生成代码来进行更新。
