...
* **🛡️ `noexcept_safe_call_and_log` (无异常安全调用与日志记录)**
    - **作用**:作为一个通用的函数模板，用于安全地调用可能抛出异常的函数 f，并确保在发生异常时能记录日志，同时防止异常逃逸出
      noexcept 函数（如析构函数）。
    - **异常处理**:
        - **捕获 std::exception 及其子类**:记录 error 级别的日志，并输出 e.what()。
        - **捕获所有其他未知异常 (...)**:记录 fatal 级别的日志。
