...
* **❄️ 雪花算法ID生成器（`snowflake_generator`）**
    ...
    - **核心原理**：基于Twitter的Snowflake算法，将64位ID划分为几个部分：
        - **1位符号位**：始终为0，确保ID为正数。
        - **43位时间戳**：精确到毫秒，决定了ID的趋势递增特性。使用自定义的“纪元点”（Epoch），可用寿命长达约279年。
        - **5位数据中心ID**：允许最多32个不同的数据中心或大区（Shard）部署。
        - **5位工作节点ID**：允许每个数据中心内最多部署32个相同的服务实例。
        - **10位序列号**：允许每个服务实例在同一毫秒内生成最多1024个ID。
...
* **📝 服务信息 (`service_info`)**
    ...
    - **特点**：包含 `instance_id` (实例唯一ID)、`service_name` (服务名称)、`host` (主机地址)、`game_server_id` (
      ...
      健康检查级别)和`start_server_time` (启动时间)。
    - **功能**：提供获取和设置这些信息的方法。
...
* **⚙️ 服务注册客户端 (`service_registry`)**
    ...
    - **功能**:
        - 向服务注册中心注册当前服务实例的详细信息。
        - 根据服务名称查询并获取该服务所有可用实例的列表。
        - 获取所有已注册的服务实例的完整列表。
