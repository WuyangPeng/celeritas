...
BOOST_AUTO_TEST_SUITE(service_info_suite)
    ...
    BOOST_AUTO_TEST_CASE(test_constructor_and_getters)
    {
        ...
        constexpr auto tcp_port = 8080;
        constexpr auto http_port = 8081;
        const celeritas::service_info::protocol_port_container ports{ { celeritas::server_network_type::tcp, tcp_port },
        ...
        BOOST_CHECK_EQUAL(info.get_port(celeritas::server_network_type::tcp), tcp_port);
        BOOST_CHECK_EQUAL(info.get_port(celeritas::server_network_type::http), http_port);
