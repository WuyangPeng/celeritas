...
BOOST_FIXTURE_TEST_SUITE(redis_list_commands_suite, celeritas::redis_database_session_fixture)
    ...
    BOOST_AUTO_TEST_CASE(test_async_left_push_and_pop)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            auto length = co_await list_commands.async_left_push(key, "value1");
    ...
    BOOST_AUTO_TEST_CASE(test_async_right_push_and_pop)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            auto length = co_await list_commands.async_right_push(key, "value1");
            BOOST_CHECK_EQUAL(length, 1);

            auto popped = co_await list_commands.async_right_pop(key);
            BOOST_REQUIRE(popped.has_value());
            BOOST_CHECK_EQUAL(*popped, "value1");
    ...
    BOOST_AUTO_TEST_CASE(test_async_push_many_and_get_length)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            auto l_length = co_await list_commands.async_left_push_many(key, values);
            BOOST_CHECK_EQUAL(l_length, 3);

            auto r_length = co_await list_commands.async_right_push_many(key, values);
            BOOST_CHECK_EQUAL(r_length, 6);

            auto total_length = co_await list_commands.async_get_length(key);
    ...
    BOOST_AUTO_TEST_CASE(test_async_get_range)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            auto range = co_await list_commands.async_get_range(key, 0, -1);
    ...
    BOOST_AUTO_TEST_CASE(test_async_remove)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            auto removed_count = co_await list_commands.async_remove(key, 2, "a");
            BOOST_CHECK_EQUAL(removed_count, 2);

            auto length = co_await list_commands.async_get_length(key);
    ...
    BOOST_AUTO_TEST_CASE(test_async_blocking_left_pop)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            // Test timeout
            auto result_timeout = co_await list_commands.async_blocking_left_pop({ key1 }, 1);
            BOOST_CHECK(!result_timeout.has_value());

            // Test pop
            co_await list_commands.async_left_push(key1, "val1");
            auto result_pop = co_await list_commands.async_blocking_left_pop({ key1, key2 }, 1);
