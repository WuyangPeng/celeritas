...
BOOST_FIXTURE_TEST_SUITE(redis_hash_commands_suite, celeritas::redis_database_session_fixture)
    ...
    BOOST_AUTO_TEST_CASE(test_async_set_and_get)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            auto result = co_await hash_commands.async_set(key, field, value);
            BOOST_CHECK_EQUAL(result, 1);

            auto retrieved_value = co_await hash_commands.async_get(key, field);
    ...
    BOOST_AUTO_TEST_CASE(test_async_set_many_and_get_many)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            bool success = co_await hash_commands.async_set_many(key, field_values);
            BOOST_CHECK(success);

            celeritas::redis_commands::key_container fields = { "f1", "f2", "non_existent" };
            auto values = co_await hash_commands.async_get_many(key, fields);
            BOOST_REQUIRE_EQUAL(values.size(), 3);
            BOOST_CHECK_EQUAL(values[0], "v1");
            BOOST_CHECK_EQUAL(values[1], "v2");
            BOOST_CHECK(values[2].empty()); // hiredis returns nil as empty string
    ...
    BOOST_AUTO_TEST_CASE(test_async_delete_and_delete_many)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            const celeritas::redis_commands::key_value_container fv = {
                { "f1", "v1" }, { "f2", "v2" }, { "f3", "v3" }
            };
            co_await session->get_redis_key_commands().async_delete(key);
            co_await hash_commands.async_set_many(key, fv);

            auto del_count1 = co_await hash_commands.async_delete(key, "f1");
            BOOST_CHECK_EQUAL(del_count1, 1);

            auto del_count_many = co_await hash_commands.async_delete_many(key, { "f2", "f3" });
            BOOST_CHECK_EQUAL(del_count_many, 2);

            auto all_fields = co_await hash_commands.async_get_fields(key);
    ...
    BOOST_AUTO_TEST_CASE(test_async_increment_by)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            auto new_value = co_await hash_commands.async_increment_by(key, field, 5);
    ...
    BOOST_AUTO_TEST_CASE(test_async_get_all_and_get_fields)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            auto all_values = co_await hash_commands.async_get_all(key);
            ...
            BOOST_CHECK(std::find(all_fields.begin(), all_fields.end(), "f1") != all_fields.end());
            BOOST_CHECK(std::find(all_fields.begin(), all_fields.end(), "f2") != all_fields.end());
    ...
    BOOST_AUTO_TEST_CASE(test_async_get_all_by_real_key)
    {
        run([this]() -> boost::asio::awaitable<void> {
            ...
            const std::string real_key = session->get_prefixed_key(key);
            const celeritas::redis_commands::key_value_container fv = {
                { "f1", "v1" }, { "f2", "v2" }
            };
            ...
            auto all_values = co_await hash_commands.async_get_all_by_real_key(real_key);
            BOOST_REQUIRE(all_values.has_value());
            BOOST_REQUIRE_EQUAL(all_values->size(), 2);
            BOOST_CHECK_EQUAL(all_values->at("f1"), "v1");
            BOOST_CHECK_EQUAL(all_values->at("f2"), "v2");
