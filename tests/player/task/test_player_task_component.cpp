...
namespace
{
    struct player_task_component_fixture
    {
        player_task_component_fixture()
            : io_context_{},
              mock_player_state_{ boost::asio::make_strand(io_context_) },
              mock_pool_{ std::make_shared<celeritas::mock_database_pool>() },
              component_{ std::make_shared<celeritas::player_task_component>(&mock_player_state_) }
        {
        ...
        std::shared_ptr<celeritas::player_task_component> component_;
    };
}

BOOST_FIXTURE_TEST_SUITE(player_task_component_suite, player_task_component_fixture)

    BOOST_AUTO_TEST_CASE(test_get_player_component_type)
    {
        BOOST_CHECK_EQUAL(static_cast<int>(component_->get_player_component_type()), static_cast<int>(celeritas::player_component_type::task));
        BOOST_CHECK_EQUAL(static_cast<int>(celeritas::player_task_component::get_player_component_type()), static_cast<int>(celeritas::player_component_type::task));
    }
