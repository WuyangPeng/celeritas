...
BOOST_AUTO_TEST_SUITE(hmac_sha256_suite)
    ...
    BOOST_AUTO_TEST_CASE(test_rfc_vector_1)
    {
        const std::string key{ "Jefe" };
        const std::string data{ "what do ya want for nothing?" };
        const std::string expected{ "5bdcc146bf60754e6a042426089575c75a003f089d2739839dec58b964ec3843" };
        BOOST_CHECK_EQUAL(celeritas::hmac_sha256::calculate(key, data), expected);
    }

    BOOST_AUTO_TEST_CASE(test_rfc_vector_2)
    {
        // 20 bytes of 0x0b
        const std::string key(20, 0x0b);
        const std::string data{ "Hi There" };
        const std::string expected{ "b0344c61d8db38535ca8afceaf0bf12b881dc200c9833da726e9376c2e32cff7" };
        BOOST_CHECK_EQUAL(celeritas::hmac_sha256::calculate(key, data), expected);
    }
